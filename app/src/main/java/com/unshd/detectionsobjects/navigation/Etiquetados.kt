        ...
        Column(modifier = Modifier.fillMaxSize().padding(innerPadding)){
            ...
            if (imagen != null) {
                LaunchedEffect(imagen) {
                    vm.processBitmap(bitmap =imagen!!)
                }
            }
...
fun HeadEtiquetados() {
    Column (verticalArrangement = Arrangement.Center,modifier = Modifier.fillMaxWidth().padding(10.dp)){
        Text("Labeling Ml kit", fontSize = 25.sp, fontWeight = FontWeight.Bold)
    }
}


@Composable
fun BottomEtiquetados(vm:EtiquetadosViewModel,uri: Uri? = null) {
...
Column (modifier = Modifier.fillMaxWidth()){
    ...
    LazyColumn (Modifier.fillMaxWidth()){
        items(if (listaEtiquetados != null) listaEtiquetados!!.size else 0){
            val label = listaEtiquetados!![it]
            ElevatedCard(
                ...
                Card(modifier = Modifier
                    ...
                    Row(horizontalArrangement = Arrangement.Absolute.SpaceBetween,
                        ...
                        Text(text = label.label, fontSize = 18.sp)
                        Spacer(Modifier.width(5.dp))
                        Text(text = label.confidence.toString()+" %", fontSize = 18.sp)
                        Spacer(Modifier.width(5.dp))
                        Text(text = label.index.toString(), fontSize = 18.sp)
...
@Composable
fun BodyEtiquetados(onSelectImage: (Uri) -> Unit) {
